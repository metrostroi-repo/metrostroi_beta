...
function TRAIN_SYSTEM:Think(dT)
	...
	if self.ChangeTime and (self.Time > self.ChangeTime) and not self.SpuriousTripTimer then
		...
		if self.relay_type == "VA21-29" and (self.Value == 1.0) then	self.Train:PlayOnce("av_on","cabin")		end
		if self.relay_type == "VB-11" and (self.Value == 0.0) then	self.Train:PlayOnce("switch2","cabin")		end
		if self.relay_type == "VB-11" and (self.Value == 1.0) then	self.Train:PlayOnce("switch2","cabin")		end
