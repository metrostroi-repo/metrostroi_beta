...
function TRAIN_SYSTEM:MoscowARS(EnableARS,KRUEnabled,BPSWorking,EnableUOS,EPKActivated)
	...
	if EnableARS then
		...
		if self.Overspeed and not self.ARSBrake then
			...
			timer.Simple(2.4 ,function() self.PneumaticBrake1 = false end)
			self.PV1Timer = CurTime() - (self.SpeedLimit <= 35 and 2 or 0)
		end
...
function TRAIN_SYSTEM:Think(dT)
	...
	if EnableALS --[[or EnableUOS]] then
		...
		local Vlimit = 0
		local VLimit2
		if self.Signal40 then Vlimit = 38 end
		if self.Signal60 then Vlimit = 58 end
		if self.Signal70 then Vlimit = 68 end
		if self.Signal80 then Vlimit = 78 end
	...
	if EPKActivated then
		...
		if (self.EPKOffARS or self.EPKTimer3) and not Train.Pneumatic.EmergencyValveEPK then
			...
			if self.EPKOffARS then
				RunConsoleCommand("say","ARS MP EPV braking (Was the emergency brake)",Train:GetDriverName())
				self.BeOffARS = nil
			else
				RunConsoleCommand("say","ARS MP EPV braking (10-30 sec has not passed)",Train:GetDriverName())
				self.BeOffARS = nil
			end
		end
		self.EPKTimer3 = nil
		...
		if (not EnableARS or (self.EPKTimer and self.EPKTimer < CurTime())) and not Train.Pneumatic.EmergencyValveEPK then
			Train.Pneumatic.EmergencyValveEPK = true
			if not EnableARS then
				RunConsoleCommand("say","ARS MP EPV braking (ARS disabled)",Train:GetDriverName())
				self.BeOffARS = true
			else
				RunConsoleCommand("say","ARS MP EPV braking (LKT not light-up on ARS stopping)",Train:GetDriverName())
				self.BeOffARS = nil
			end
		end
	else
