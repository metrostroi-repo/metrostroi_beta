...
function TRAIN_SYSTEM:ClientThink(dT)
	...
	local pitch = 1
	-- Play horn sound
	self.Train:SetSoundState(self.Train:GetNWBool("HornType",false) and "horn3" or "horn2",self.Active and absoluteVolume or 0,absolutePitch*pitch,nil,1.09)
	if (self.Active ~= active) and (not active) then
		if absolutePitch > 0.2 then
			self.Train:PlayOnce(self.Train:GetNWBool("HornType",false) and "horn3_end" or "horn2_end","cabin",1.09,101.5*absolutePitch*pitch) --0.85
		end
	end
