...
function ENT:Think()
	...
	if not self.ARSOnly then
		...
		for k,v in ipairs(self.LensesTBL) do
			...
			if v ~= "M" then
				if k ~= 4 and data[3] and data[3].glass then
					if type(data[3].glass) == "table" then
						for i,tbl in pairs(data[3].glass) do
							if not IsValid(self.Models[1][tostring(ID).."_glass"..i]) then	--NEWLENSES
								...
								self.Models[1][tostring(ID).."_glass"..i]:SetPos(self:LocalToWorld(self.BasePosition + offset + tbl[2]))
								self.Models[1][tostring(ID).."_glass"..i]:SetAngles(self:LocalToWorldAngles(Angle(0,self.Left and -90 or 90,0)))
								self.Models[1][tostring(ID).."_glass"..i]:SetParent(self)
							end
						end
					else
						if not IsValid(self.Models[1][tostring(ID).."_glass"]) then	--NEWLENSES
							self.Models[1][tostring(ID).."_glass"] = ClientsideModel(data[3].glass,RENDERGROUP_OPAQUE)
							self.Models[1][tostring(ID).."_glass"]:SetPos(self:LocalToWorld(self.BasePosition + offset + Vector(0,7,0)))
							self.Models[1][tostring(ID).."_glass"]:SetAngles(self:LocalToWorldAngles(Angle(0,self.Left and 0 or 180,0)))
							self.Models[1][tostring(ID).."_glass"]:SetParent(self)
						end
					end
				end
				for i = 1,#v do
					...
					if State >0 and self.Signals[ID][i] ~= State  and not IsValid(self.Models[3][ID..ID2]) then
						self.Models[3][ID..ID2] = ClientsideModel("models/cyber_metrostroi/test/lamp.mdl",RENDERGROUP_OPAQUE)	--NEWLENSES
						...
						self.Models[3][ID..ID2]:SetAngles(self:LocalToWorldAngles(Angle(0,90*(self.Left and -1 or 1),0)))--NEWSIGNALS
