...
ENT.ButtonMap["LAMPS"] = {
	...
	buttons = {
		...
		{ID = "GreenRP",x=233,y=60, radius=10, tooltip="Лампа СН: Сигнализация неисправности (Сигнализация перегрузки)\nRP: SN overload relay light (overload relay open on current train)"},
		{ID = "RedRP",	x=255,y=58.8, radius=10, tooltip="Красная РП: Красная лампа реле перегрузки\nRP: Red overload relay light (power circuits failed to assemble)"},
		{ID = "Blue",		x=170,y=58.8, radius=20, tooltip="Лампа СД: Сигнализация дверей поезда\nSD door state light (doors are open)"},
		{ID = "RedRP2", 				x=213.5, y=58.75, radius=10, tooltip="РП поезда: Лампа реле перегрузки\nTrain RP: Overload relay light (power circuits failed to assemble)"},
...
ENT.ButtonMap["Back1"] = {
	...
	{x=38, y=145, w=180, h=120, tooltip="Щиток с низковольтными предохранителями\nShield with low-voltage fuses"},
	}
}
...
Metrostroi.ClientPropForButton("RedRP2",{
	panel = "LAMPS",
	button = "RedRP2",
	model = "models/metrostroi_train/81-508/508_rp2_lamp.mdl",
	z=0,
...
function ENT:UpdateTextures() print ("1")
...
function ENT:Think()
	...
	self:ShowHideSmooth("RedRP2",self:Animate("RedRP_hs",self:GetPackedBool(36) and 1 or 0,0,1,5,false) + self:Animate("RedLSN_hs",self:GetPackedBool(131) and 1 or 0,0,0.4,5,false))
	self:ShowHideSmooth("Blue",self:Animate("Doors_hs",self:GetPackedBool(40) and 0 or 1,0,1,5,false))
	self:ShowHideSmooth("Red",self:Animate("light_PECH",self:GetPackedBool(37) and 1 or 0,0,1,5,false))
	self:ShowHideSmooth("UKS",self:Animate("AVUl",self:GetPackedBool(38) and 0 or 1,0,1,5,false))
	--self:ShowHideSmooth("AVU",self:Animate("AVUl",self:GetPackedBool(38) and 1 or 0,0,1,5,false))
	...
	self:SetSoundState("cran1",self:GetPackedBool("DriverValveTLDisconnect") and 1 or 0,1.0)
   ...
   -- self:Animate("speed1", 			self:GetPackedRatio("Speed") + math.sin(math.pi*8*self.Angle)*2/120,			0.495, 0.746,				nil, nil,  256,2,0.01)
	...
	--- Чтобы лампы не загорались с выкл БВ
		self:ShowHide("Blue",self:GetPackedBool(32))
		self:ShowHide("UKS",self:GetPackedBool(32))
	-- Compressor
