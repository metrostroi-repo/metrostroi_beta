...
function ENT:Initialize()
	...
	self.DriverSeat = self:CreateSeat("driver",Vector(425+6,-39,-27.5),Angle(0,0,0))
	...
	-- Create bogeys
	self.FrontBogey = self:CreateBogey(Vector( 315,0,-84.5),Angle(0,180,0),true)
	self.RearBogey  = self:CreateBogey(Vector(-315,0,-84.5),Angle(0,0,0),false)
	...
	self.Lights = {
		...
		-- Cabin
		[10] = { "dynamiclight",	Vector(435,0,20), Angle(0,-0,0), Color(219,179,71), brightness = 0.005, distance = 600, shadows = 1},

		-- Interior
		[11] = { "dynamiclight",	Vector( 250, 0, 0), Angle(0,0,0), Color(255,140,18), brightness = 6, distance = 300 , fov=180,farz = 128 },
		[12] = { "dynamiclight",	Vector(   0, 0, 0), Angle(0,0,0), Color(255,140,18), brightness = 6, distance = 400, fov=180,farz = 128 },
		[13] = { "dynamiclight",	Vector(-300, 0, 0), Angle(0,0,0), Color(255,140,18), brightness = 5, distance = 400 , fov=180,farz = 128 },
...
function ENT:Think()
	...
	if jerk > (2.0 + self.Speed/15.0) then
		...
		if ((math.random() > 0.00) or (jerk > 10)) and (CurTime() - self.PrevTriggerTime1 > 1.5) then
			...
			self.FrontBogey:EmitSound("subway_trains/ezh/chassis_"..math.random(1,3)..".wav", 100, math.random(90,110))
		end
		if ((math.random() > 0.00) or (jerk > 10)) and (CurTime() - self.PrevTriggerTime2 > 1.5) then
			self.PrevTriggerTime2 = CurTime()
			self.RearBogey:EmitSound("subway_trains/ezh/chassis_"..math.random(1,3)..".wav", 100, math.random(90,110))
		end
	end
