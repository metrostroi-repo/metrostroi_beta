...
function ENT:ShouldRenderClientEnts()
	return self:GetPos():Distance(LocalPlayer():GetPos()) < 960*5 or LocalPlayer():GetPos().z - self:GetPos().z < 500
end
...
function ENT:Draw()
	...
	if not self.CLDraw then
		hook.Add("PostDrawOpaqueRenderables", "metrostroi_base_draw_"..self:EntIndex(), function()
			...
			if self.ButtonMap and self.ButtonMap["Schedule"] then
