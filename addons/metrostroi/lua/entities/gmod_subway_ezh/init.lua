...
function ENT:Initialize()
	...
	self:SetModel("models/metrostroi_train/81-707/81-707.mdl")
	...
	-- Create bogeys
	self.FrontBogey = self:CreateBogey(Vector( 320,0,-84.5),Angle(0,180,0),true)
	self.RearBogey  = self:CreateBogey(Vector(-321.5,0,-84.5),Angle(0,0,0),false)
	...
	self.Lights = {
		-- Head
		[1] = { "headlight",		Vector(468,0,-35), Angle(20,0,0), Color(240,120,60),shadows = 0, fov = 90 },
		...
		-- Interior
		[11] = { "dynamiclight",	Vector( 250, 0, -5), Angle(180,0,0), Color(255, 176, 59), brightness = 6, distance = 450 , fov=180,farz = 128 },
		[12] = { "dynamiclight",	Vector(   0, 0, -5), Angle(180,0,0), Color(255, 176, 59), brightness = 6, distance = 400, fov=180,farz = 128 },
		[13] = { "dynamiclight",	Vector(-300, 0, -5), Angle(180,0,0), Color(255, 176, 59), brightness = 6, distance = 400 , fov=180,farz = 128 },
...
function ENT:Think()
	...
	-- Секвенция
	self:SetPackedBool("Sequence",(self.RPU.Value == 1.0) and (self:ReadTrainWire(1) > 0.5 or self:ReadTrainWire(6) > 0.5))
	...
	if (self.ALS_ARS.Ring == true) then
		if CurTime() + 3 then self:SetPackedBool(39,true) end
	end
...
function ENT:BrokePlomb(but,nosnd)
	...
	if not nosnd then
	 if but == "KAH" then
		...
		RunConsoleCommand("say",drv.." broke seal on Ezh`s ARS-MP TAH!")
		end 
	if but == "RC1" then
		hook.Run("MetrostroiPlombBroken",self,but,drv)
		RunConsoleCommand("say",drv.." broke seal on Ezh`s ARS-MP RCR!")
		end
	if but == "OtklAVU" then
		hook.Run("MetrostroiPlombBroken",self,but,drv)
		RunConsoleCommand("say",drv.." broke seal on AVU!")
		end 
	if but == "UOS" then
		hook.Run("MetrostroiPlombBroken",self,but,drv)
		RunConsoleCommand("say",drv.." broke seal on Ezh`s ARS-MP UOS!")
		end 
	if but == "ALS" then
		hook.Run("MetrostroiPlombBroken",self,but,drv)
		RunConsoleCommand("say",drv.." broke seal on Ezh`s ARS-MP ALS!")
		end
	if but == "PD" then
		hook.Run("MetrostroiPlombBroken",self,but,drv)
		RunConsoleCommand("say",drv.." broke seal on Ezh`s ARS-MP PD!")
		end 
	if but == "RST" then
		hook.Run("MetrostroiPlombBroken",self,but,drv)
		RunConsoleCommand("say",drv.." broke seal on RST!")
		end 
	end
end
--------------------------------------------------------------------------------
function ENT:OnButtonPress(button,route)
	...
	if button == "VRDToggle" then
		...
		RunConsoleCommand("say",drv.." "..state.." Устройство Ограничения скорости (UOS)!")
	end
	if button == "RC1Toggle" then
		local drv = self:GetDriverName()
		local state = self.RC1.TargetValue < 0.5 and "enabled" or "disabled"
		RunConsoleCommand("say",drv.." "..state.." RCR!")
	end 
		if button == "KAHToggle" then
		local drv = self:GetDriverName()
		local state = self.KAH.TargetValue < 0.5 and "enabled" or "disabled"
		RunConsoleCommand("say",drv.." "..state.." TAH!")
	end
