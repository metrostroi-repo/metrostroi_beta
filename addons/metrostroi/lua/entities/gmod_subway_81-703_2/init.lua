...
function ENT:Initialize()
	...
	self:SetModel("models/metrostroi_train/81-703/81-703_interim.mdl")
	self.BaseClass.Initialize(self)
	self:SetPos(self:GetPos() + Vector(0,0,140))

	-- Create seat entities
	self.DriverSeat = self:CreateSeat("driver",Vector(427,-39,-24.5),Angle(0,0,0))
	...
	self.Lights = {
		...
		-- Cabin
		[10] = { "dynamiclight",	Vector(435,0,20), Angle(0,-0,0), Color(230,184,111), brightness = 0.004, distance = 600, shadows = 1},

		-- Interior
		[11] = { "dynamiclight",	Vector( 250, 0, 0), Angle(0,0,0), Color(230,184,111), brightness = 5, distance = 300 , fov=180,farz = 128 },
		[12] = { "dynamiclight",	Vector(   0, 0, 0), Angle(0,0,0), Color(230,184,111), brightness = 5, distance = 400, fov=180,farz = 128 },
		[13] = { "dynamiclight",	Vector(-300, 0, 0), Angle(0,0,0), Color(230,184,111), brightness = 5, distance = 400 , fov=180,farz = 128 },
...
function ENT:Think()
	...
	if jerk > (2.0 + self.Speed/15.0) then
		...
		if ((math.random() > 0.00) or (jerk > 10)) and (CurTime() - self.PrevTriggerTime1 > 1.5) then
			...
			self.FrontBogey:EmitSound("subway_trains/ezh/chassis_"..math.random(1,3)..".wav", 70, math.random(90,110))
		end
		if ((math.random() > 0.00) or (jerk > 10)) and (CurTime() - self.PrevTriggerTime2 > 1.5) then
			self.PrevTriggerTime2 = CurTime()
			self.RearBogey:EmitSound("subway_trains/ezh/chassis_"..math.random(1,3)..".wav", 70, math.random(90,110))
		end
	end
...
function ENT:OnButtonPress(button)
	...
	if button == "DriverValveDisconnect" then
		if self.DriverValveBLDisconnect.Value == 0 or self.DriverValveTLDisconnect.Value == 0 then
			...
			self:PlayOnce("epv_start","cabin",0.9)
			else
		...
		if self.DriverValveBLDisconnect.Value == 1.0 then
			self:PlayOnce("pneumo_disconnect1","cabin",0.9)
		if self.DriverValveBLDisconnect.Value == 1 then self:PlayOnce("epv_off","cabin",0.9) end
		else
		--self:PlayOnce("pneumo_disconnect2","cabin",0.9)
