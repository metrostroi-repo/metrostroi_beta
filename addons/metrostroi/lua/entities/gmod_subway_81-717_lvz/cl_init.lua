...
ENT.ButtonMap["Stopkran"] = {
	...
	scale = 0.1/2,
		buttons = {
			{ID = "STOPKRANToggle",x=0, y=0, w=200, h=1300, tooltip="ЖЕНЯ ГДЕ ПУИ И ВСЁ ГОВНО(это )"},
	}
}
...
function ENT:Think()
	...
	if self.Breakers ~= self:GetNW2Bool("Breakers") then
		...
		self:ShowHide("VRD_2",self.Breakers and self.Blok and (self.Blok == 1 or self.Blok == 3))
	end
...
--[[
	...
	-- ARS/ringer alert
	state = self:GetPackedBool(39) or self:GetPackedBool(164)
	...
	if self.PreviousAlertState ~= state then
		...
		if state then
			self:SetSoundState("ring",1,1)
		else
			self:SetSoundState("ring",0,0)
			self:SetSoundState("ring_end",0,1)
			self:SetSoundState("ring_end",1,1)
		end
	end
...
function ENT:DrawPost(special)
	...
	self:DrawOnPanel("IGLA",function(...) self.IGLA:IGLA(...) end)
